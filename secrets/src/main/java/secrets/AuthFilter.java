			...
			if (cookie.getName().equals(AUTH_COOKIE_NAME)) {
				try {
					...
					con.setRequestProperty("Cookie",
							AUTH_COOKIE_NAME + "=" + cookie.getValue());
					...
					if (status >= 200 && status < 300) {
						...
						JSONObject perms = jsonResponse.optJSONObject("permissions",
								new JSONObject());
						if (perms.has("allowSecretsAccess") && !perms
								.optBoolean("allowSecretsAccess", false)) {
							HttpUtil.sendSimpleJson(resp, 401, "error",
						...
						JSONObject jsonResponse = new JSONObject(
								responseBody.toString());
						HttpUtil.sendSimpleJson(resp, status,
								jsonResponse.optString("type", "error"),
